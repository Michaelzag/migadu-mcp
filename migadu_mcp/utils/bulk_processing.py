...
async def log_bulk_operation_start(ctx: Context, operation: str, count: int, entity_type: str):
...
async def log_bulk_operation_result(ctx: Context, operation: str, result: Dict[str, Any], entity_type: str):
    ...
    if total == 1:
        ...
        if failed == 0:
            await ctx.info(f"✅ {operation} completed: {successful}/{total} {entity_type}s processed successfully")
        else:
            await ctx.warning(f"⚠️ {operation} completed with issues: {successful}/{total} {entity_type}s succeeded, {failed} failed")


def validate_required_fields(item: Dict[str, Any], required_fields: List[str], operation: str) -> None:
    """Validate that required fields are present in the item dict"""
    missing_fields = [field for field in required_fields if field not in item]
    if missing_fields:
        raise ValueError(f"{operation} requires fields: {', '.join(missing_fields)}. Missing: {', '.join(missing_fields)}")


def get_field_with_default(item: Dict[str, Any], field: str, default: Any = None) -> Any:
    """Get field from dict with default value if not present"""
    return item.get(field, default)


def validate_with_schema(data: Dict[str, Any], schema_class: Type[BaseModel]) -> BaseModel:
