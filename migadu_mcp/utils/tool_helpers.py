    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> Dict[str, Any]:
            ...
            if ctx and isinstance(result, dict):
                ...
                summary_keys = [key for key in protected_result.keys() if key.endswith('_summary')]
                if summary_keys:
                    await ctx.info("📊 Response summarized. Use specific get_* functions for details.")
...
async def log_operation_success(ctx: Context, operation: str, target: str, count: Optional[int] = None):
