...
def truncate_response_if_needed(data: Dict[str, Any], max_tokens: int = 2000) -> Dict[str, Any]:
    """
...
def _create_list_summary(data: Dict[str, Any], list_key: str) -> Dict[str, Any]:
    ...
    summary = {
        f"{list_key}_summary": {
            ...
            "estimated_tokens": estimate_token_count(data),
            "truncated": True,
            "message": f"Response truncated to prevent context explosion. Use specific get_ commands for details.",
        }
    }
...
def _summarize_mailboxes(mailboxes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    active_count = sum(1 for mb in mailboxes if mb.get('may_receive', True) and mb.get('may_send', True))
...
def _summarize_aliases(aliases: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        "breakdown": {
            ...
            "avg_destinations": round(sum(dest_counts) / len(dest_counts) if dest_counts else 0, 1),
        },
        "addresses": sorted(addresses),
...
def _summarize_identities(identities: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    receive_enabled = sum(1 for identity in identities if identity.get('may_receive', True))
...
def _create_generic_summary(data: Dict[str, Any], estimated_tokens: int, max_tokens: int) -> Dict[str, Any]:
